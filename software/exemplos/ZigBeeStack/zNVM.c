 ...
 ********************************************************************/
    ...
    BYTE NVMInit( void )
    {
        ...
        #if defined(USE_EXTERNAL_NVM) && defined(STORE_MAC_EXTERNAL)
            result |= NVMalloc(sizeof (LONG_ADDR), &macLongAddr );
        #endif
        ...
        if (!result)
        {
            ...
            {
                // NOTE - Currently, we need a simple descriptor for the ZDO endpoint.  When this requirement
                // goes away, we need to take off the "+1".
                int     i;
 ...
 ********************************************************************/
    ...
    void NVMRead (BYTE *dest, WORD  src, BYTE count)
    {
        ...
        #ifdef ENABLE_DEBUG
            ConsolePut('r');
//            ConsolePutROMString((ROM char * const)"NVMRead from ");
