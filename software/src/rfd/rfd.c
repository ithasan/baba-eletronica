   ...
   static union
{
   struct
   {
      BYTE    bAudioSensorButtonPressed    : 1;
         ...
         case NO_PRIMITIVE:
             ...
             *******************************************************************************************/
               ...
               if (ZigBeeStatus.flags.bits.bDataRequestComplete && ZigBeeReady())
               {
                  ...
                  {
                     // reinicia a flag indicativa de botao/sensor acionado

                     myStatusFlags.bits.bMoveSensorButtonPressed = FALSE;

                     // Envia mensagem para o coordinator indicando um movimento detectado
                     RFDSendMessage(MoveSensor_Activated, 0x00);                
                  }
                  else if (myStatusFlags.bits.bAudioSensorButtonPressed)
                  {
                     // reinicia a flag indicativa de botao/sensor acionado

                     myStatusFlags.bits.bAudioSensorButtonPressed = FALSE;

                     // Envia mensagem para o coordenador indicando um audio detectado

                     RFDSendMessage(AudioSensor_Activated, 0x00);                                
                  }
...
BOOL myProcessesAreDone( void )
{
   return (myStatusFlags.bits.bAudioSensorButtonPressed==FALSE) && (myStatusFlags.bits.bMoveSensorButtonPressed==FALSE);
}
...
void UserInterruptHandler(void)
{
   ...
   if ( INTCONbits.RBIF == 1 )
   {
      ...
      if (LEVEL_SENSOR == 0)
      {
         myStatusFlags.bits.bAudioSensorButtonPressed = TRUE;
