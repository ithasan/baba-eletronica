...
#define LIGHT_TOGGLE                0xF0
   ...
   /*******************************************************************************
    *    Prototipos das funcoes BBE - Baba Eletronica
    *******************************************************************************/

   // gera um atraso de tempo aproximado

   void BBE_Delay( void );

   // liga o led indicativo de movimento

   void BBE_TurnLedOn( void );

   // desliga o led indicativo de movimento

   void BBE_TurnLedOff( void );

   // liga o buzzer indicativo de audio detectado

   void BBE_TurnBuzzerOn( void );

   // desliga o buzzer indicativo de audio detectado

   void BBE_TurnBuzzerOff( void );

   // Alerta um movimento detectado pelo RFD.

   void BBE_AlertMoveDetected( void );

   // Alerta audio detectado pelo RFD.

   void BBE_AlertAudioDetected( void );
...
{    
   ...
   LED_PIN = !myStatusFlags.bits.bIsBound;
   LATAbits.LATA4 = 0;
                  ...
                  case EP_LIGHT:
                     if ((frameHeader & APL_FRAME_TYPE_MASK) == APL_FRAME_TYPE_KVP)
                     {
                        ...
                        for (transaction=0; transaction<frameHeader; transaction++)
                        {
                            ...
                            *******************************************************************************************
                            **                                                                                       **
                            **                       CÓDIGO ADAPTADO POR: Daniel Gonçalves                           **
                            **                                                                                       ** 
                            *******************************************************************************************
                            *******************************************************************************************/
                           if(params.APSDE_DATA_indication.ClusterId == BabyControl_CLUSTER)
                           {
                              if(command == APL_FRAME_COMMAND_SET || command == APL_FRAME_COMMAND_SETACK)
                              {
                                 ...
                                 {
                                    case AudioSensor_Activated:
                                       {
                                          // Alerta uma deteccao de audio pelo RFD 

                                          BBE_AlertAudioDetected();

                                          // Envia uma mensagem para saida serial

                                          ConsolePutROMString((ROM char *) "Audio Detectado\r\n");
                                       }break;
                                    case MoveSensor_Activated:
                                       {
                                          // Alerta uma deteccao de movimento pelo RFD

                                          BBE_AlertMoveDetected();

                                          // Envia uma mensagem para saida serial

                                          ConsolePutROMString((ROM char *) "Movimento Detectado\r\n");
                                       }break;
                                 }//switch
                              }// if
